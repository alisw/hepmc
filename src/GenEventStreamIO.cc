...
namespace HepMC {
  ...
  std::istream& GenEvent::read( std::istream& is )
  {
    ...
    if ( signal_process_vertex ) {
      set_signal_process_vertex( barcode_to_vertex(signal_process_vertex) );
    }
    //
  ...
  namespace detail {
                                  ...
                                  GenParticle * p )
    {
      ...
      iline >> bar_code;
      if (!iline) {  delete p; throw IO_Exception("read_particle input stream encountered invalid data"); }
      iline >> id;
      if (!iline) {  delete p; throw IO_Exception("read_particle input stream encountered invalid data"); }
      iline >> px;
      if (!iline) {  delete p; throw IO_Exception("read_particle input stream encountered invalid data"); }
